...
void AddRecord()
{
	...
	int Price;
	BOOL Korean;
	SQLINTEGER lenName=SQL_NTS,lenPrice=0,lenKorean=0;
	int i;

	srand(LOWORD(GetCurrentTime()));
	if (SQLPrepare(hStmt,(SQLCHAR *)"Insert into tblCigar (name,price,korean)"
	...
	for (i=0;i<5;i++) {
		wsprintf((LPTSTR)Name,"담배%d",rand());
...
void UpdateRecord()
{
	...
	if (SQLExecDirect(hStmt,(SQLCHAR *)szSQL,SQL_NTS))
		return;
