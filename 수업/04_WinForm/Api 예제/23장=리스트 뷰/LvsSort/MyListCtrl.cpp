...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		for (i=0;i<sizeof(people)/sizeof(people[0]);i++) {
			...
			ListView_SetItemText(hList,idx,1,people[i].tel);
			ListView_SetItemText(hList,idx,2,people[i].addr);
		}
		...
		CreateWindow("button","남자",WS_CHILD | WS_VISIBLE | 
	...
	case WM_COMMAND:
		...
		case IDC_DEL:
			...
			if (idx == -1)  {
				MessageBox(hWnd,"삭제할 항목을 먼저 선택하십시오.","알림",MB_OK);
			} else {
...
//*/
		...
		case IDC_EDIT:
			idx=ListView_GetNextItem(hList,-1,LVNI_ALL | LVNI_SELECTED);
			if (idx == -1)  {
				MessageBox(hWnd,"수정할 항목을 먼저 선택하십시오.","알림",MB_OK);
			} else {
				LI.mask=LVIF_IMAGE;
				LI.iItem=idx;
				LI.iSubItem=0;
