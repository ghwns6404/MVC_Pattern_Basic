...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		// 아무런 지정이 없을 때는 더블 클릭해야 활성화된다.
		// ListView_SetExtendedListViewStyle(hList, 0);

		// 마우스 커서가 항목 위에 있으면 자동으로 선택된다.
		//ListView_SetExtendedListViewStyle(hList, LVS_EX_TRACKSELECT);
		//ListView_SetHoverTime(hList, 10);

		// 한번 클릭으로 활성화된다.
		//ListView_SetExtendedListViewStyle(hList, LVS_EX_ONECLICKACTIVATE);

		// 한번 클릭으로 활성화되며 핫 항목에 밑줄을 긋는다.
		//ListView_SetExtendedListViewStyle(hList, LVS_EX_ONECLICKACTIVATE | LVS_EX_UNDERLINEHOT);

		// 두번 클릭으로 활성화된다. 더블클릭과는 조금 다르다.
		//ListView_SetExtendedListViewStyle(hList, LVS_EX_TWOCLICKACTIVATE);

		// 두번 클릭으로 활성화되며 활성화될 항목에 밑줄을 긋는다.
		//ListView_SetExtendedListViewStyle(hList, LVS_EX_TWOCLICKACTIVATE | LVS_EX_UNDERLINECOLD);

		// 두번 클릭으로 활성화되며 활성화될 핫 항목에 밑줄을 긋는다.
		//ListView_SetExtendedListViewStyle(hList, LVS_EX_TWOCLICKACTIVATE | LVS_EX_UNDERLINEHOT);
		return 0;
	case WM_NOTIFY:
		LPNMHDR hdr;
		...
		if (hdr->hwndFrom == hList) {
			...
			case LVN_ITEMACTIVATE:
				wsprintf(str, "%d 항목이 활성화되었음",nia->iItem);
