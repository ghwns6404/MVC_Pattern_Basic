...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		for (i=0;i<sizeof(people)/sizeof(people[0]);i++) {
			...
			ListView_SetItemText(hList,i,1,LPSTR_TEXTCALLBACK);
			ListView_SetItemText(hList,i,2,LPSTR_TEXTCALLBACK);
		}
		return 0;
	case WM_NOTIFY:
		LPNMHDR hdr;
		...
		if (hdr->hwndFrom == hList) {
			...
			case LVN_GETDISPINFO:
				if ((ndi->item.mask & LVIF_IMAGE) == LVIF_IMAGE) {
					ndi->item.iImage=(people[ndi->item.iItem].male ? 0:1);
				}
				if ((ndi->item.mask & LVIF_TEXT) == LVIF_TEXT) {
					switch (ndi->item.iSubItem) {
					case 0:
						ndi->item.pszText=people[ndi->item.iItem].name;
						break;
					case 1:
						ndi->item.pszText=people[ndi->item.iItem].tel;
						break;
					case 2:
						ndi->item.pszText=people[ndi->item.iItem].addr;
						break;
					}
				}
				return TRUE;
