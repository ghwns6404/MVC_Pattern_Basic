...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	TVITEMEX TvEx;
	tag_Param *pParam;
	static TCHAR strInfo[255]="조사하고자 하는 지역을 선택하십시오.";
	...
	case WM_NOTIFY:
		...
		if (hdr->hwndFrom == hTree) {
			...
			case TVN_SELCHANGED:
				...
				if (TvEx.iImage != 3) {
					lstrcpy(strInfo,"하위 항목을 선택해 주십시오");
				} else {
					wsprintf(strInfo,"'%s'의 우편번호는 %s입니다.",
						Cap, pParam->Num);
				}
			...
			case TVN_SELCHANGING:
				...
				TvEx.hItem=ntv->itemNew.hItem;
				TreeView_GetItem(hTree,&TvEx);
				if (lstrcmp(Cap,"신사동") == 0) {
					MessageBox(hWnd,"선택하신 항목의 정보가 없습니다","알림",MB_OK);
					return TRUE;
				} else {
					return FALSE;
				}
	...
	case WM_LBUTTONDOWN:
		...
		if (hNow==NULL) {
			...
			TvEx.hItem=hNow;
			TreeView_GetItem(hTree,&TvEx);
			wsprintf(strInfo,"'%s' 항목이 선택되어 있습니다",Cap);
		}
		InvalidateRect(hWnd,NULL,TRUE);
		return 0;
	case WM_PAINT:
		hdc=BeginPaint(hWnd, &ps);
		TextOut(hdc,250,50,strInfo,lstrlen(strInfo));
		EndPaint(hWnd, &ps);
		return 0;
	case WM_DESTROY:
