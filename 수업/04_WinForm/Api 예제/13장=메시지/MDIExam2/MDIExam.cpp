...
LRESULT CALLBACK ClientSubProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	PAINTSTRUCT ps;
	HDC hdc;
	int x,y;
	...
	case WM_PAINT:
		...
		GetObject(hClientBack,sizeof(BITMAP),&bm);
		for (y=0;y<rt.bottom;y+=bm.bmHeight) {
			for (x=0;x<rt.right;x+=bm.bmWidth) {
				DrawBitmap(hdc,x,y,hClientBack);
			}
		}
		EndPaint(hWnd, &ps);
		return 0;
	}
	return CallWindowProc(OldClientProc,hWnd,iMessage,wParam,lParam);
}
...
LRESULT CALLBACK MDIWndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_DESTROY:
		SetWindowLongPtr(g_hMDIClient,GWLP_WNDPROC,(LONG_PTR)OldClientProc);
		DeleteObject(hClientBack);
		PostQuitMessage(0);
		return 0;
	}
